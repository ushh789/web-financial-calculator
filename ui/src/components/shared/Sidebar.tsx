...
import { LayoutDashboard, Calculator, FileText, ShieldCheck, Settings } from "lucide-react";
...
function NavItem({
  ...
  return (
    <Link
      href={href}
      className={clsx(
        "flex items-center gap-2.5 px-3 py-2 rounded-[--radius-sm] text-sm transition-colors relative",
        isActive
...
export function Sidebar() {
  ...
  return (
    <aside className="w-[248px] shrink-0 border-r border-[--border] bg-[--bg-tint] flex flex-col min-h-screen">
      {/* Brand header */}
      <div className="p-4 border-b border-[--border] flex items-center gap-2.5">
        <div className="w-5 h-5 flex items-center justify-center rounded bg-[--accent] shrink-0">
          <span className="font-serif font-medium text-[13px] text-[--accent-fg] leading-none">A</span>
        </div>
        <div className="flex flex-col min-w-0">
          <span className="text-sm font-medium text-[--text] leading-tight">Aurelius</span>
          <span className="text-xs text-[--text-3] leading-tight">Finance Studio</span>
        </div>
      </div>
