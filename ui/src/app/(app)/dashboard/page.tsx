    ...
    <div className="space-y-8">
      ...
      <div className="bg-surface rounded-lg border border-border shadow-1 px-8 py-7 flex items-center justify-between gap-8">
        <div className="min-w-0">
          <p className="text-[11px] uppercase tracking-[0.08em] text-text-2 font-medium mb-2">
            {t("title")}
          </p>
        ...
        <div className="hidden lg:flex flex-col items-end shrink-0">
          ...
          <span className="text-sm text-text-2 mt-1">{t("totalCalculations")}</span>
        </div>
      </div>
          ...
          {isLoading ? (
            ...
            <div className="rounded-lg border border-dashed border-border p-10 flex flex-col items-center gap-4 text-center">
              <div className="w-10 h-10 rounded-full bg-surface-sunken flex items-center justify-center">
                <Calculator className="h-5 w-5 text-text-2" />
              </div>
              <p className="text-sm text-text-2">{t("noCalculations")}</p>
              <Link
                href="/calculators"
                ...
                {activeCalculators.map((calc) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] font-mono text-text-2 mt-0.5">{calc.code}</p>
                    </div>
                    <ArrowRight className="w-4 h-4 text-text-2 group-hover:text-accent transition-colors shrink-0" />
                  </Link>
                ))}
              </div>
            )}
